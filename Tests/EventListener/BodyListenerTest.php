...
class BodyListenerTest extends \PHPUnit_Framework_TestCase
{
    ...
    /**
     * @dataProvider contentTypeProviders
     */
    public function testIfContentTypeIsNotEmptyWhenIsNotPassedByRequest($defaultFormat, $expected, $contentType = null)
    {
        $request = new Request();
        ...
        $listener = new BodyListener($decoderProvider, false, null, true);
        $listener->setDefaultFormat($defaultFormat);
        $listener->onKernelRequest($event);

        $this->assertNotEmpty($request->getContentType());
        $this->assertSame($request->getContentType(), $expected);
    }

    /**
     * @return array
     */
    public function contentTypeProviders()
    {
        return [
            ['json', 'json'],
            ['html', 'html'],
